package com.mymain.appcertificacao.codelab.datamanagement.sharedprefs
...
import com.mymain.appcertificacao.databinding.FragmentSharedPrefBinding
...
class SharedPrefFragment : Fragment(R.layout.fragment_shared_pref) {
    ...
    companion object {
        ...
        //Indentidade para ele ser unico
        const val SHARED_PREFS = "com.sofsuit.codelab.datamanagement.sheredprefs"
    }

    private var mCount = 0

    @ColorRes
    private var mColor = R.color.black_text_color
    ...
    override fun onPause() {
        super.onPause()
        // NA HORA DO "APERREIO" DA PROVA, DA UM SWITCH PRACÁ, LEMBRA E VOLTA!
        mPreferences.let {
            val editor = it.edit()
    ...
    private fun reset() {
        ...
        // NA HORA DO "APERREIO" DA PROVA, DA UM SHITCH PARA CÁ, LEMBRAR E VOLTA!
        mPreferences.let {
            val editor = it.edit()
            editor.clear()
            editor.apply()
    ...
    private fun getColorFromResourceId(@ColorRes colorId: Int): Int {
        return if (colorId != 0) requireActivity().getColor(colorId) else R.color.black_text_color
    }
}